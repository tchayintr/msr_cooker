...
def gen_div_data(data, data_format, shuffle=False, train=True):
    if data_format == SL_FORMAT:
        data = gen_div_data_SL(data, shuffle=shuffle, train=train)
    elif data_format == WL_FORMAT:
        data = gen_div_data_WL(data, shuffle=shuffle, train=train)
    return data


def gen_div_data_SL(data, shuffle=False, train=True):
    ls = data  # lines

    # shuffle
    if shuffle and train:
        random.Random(RANDOM_SEED).shuffle(ls)
...
def gen_div_data_WL(data, shuffle=False, train=True):
    raise NotImplementedError
    sys.exit()
...
def cook(args):
    ...
    if args.output_data_path:
        if args.unshuffle:
            output_train_data_path = '{}/cooked_msr_{}.train.seg.{}'.format(
                args.output_data_path, start_time, args.output_data_format)
            output_valid_data_path = '{}/cooked_msr_{}.valid.seg.{}'.format(
                args.output_data_path, start_time, args.output_data_format)
        else:
            output_train_data_path = '{}/cooked_msr_{}.train.shuf.seg.{}'.format(
                args.output_data_path, start_time, args.output_data_format)
            output_valid_data_path = '{}/cooked_msr_{}.valid.shuf.seg.{}'.format(
                args.output_data_path, start_time, args.output_data_format)

        # no shuffle for test data
        output_test_data_path = '{}/cooked_msr_{}.test.seg.{}'.format(
            args.output_data_path, start_time, args.output_data_format)
